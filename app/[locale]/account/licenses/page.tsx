...
export default async function AccountLicensesPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const packageLabel = (value: string) =>
    packageValues.includes(value as (typeof packageValues)[number]) ? t(`forms.packages.${value}`) : value;
              ...
              <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
                <div>
                  ...
                  <p className="mt-1 text-sm text-[var(--muted)]">
                    {packageLabel(item.currentPackage)} -&gt; {packageLabel(item.requestedPackage)}
                  </p>
                </div>
