...
export default async function AccountPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const packageLabel = (value: string | null | undefined) => {
    const packageName = value || "FREE";
    return packageValues.includes(packageName as (typeof packageValues)[number])
      ? t(`forms.packages.${packageName}`)
      : packageName;
  };
        ...
        <Card className="p-5">
          ...
          <dl className="grid gap-3 text-sm">
            ...
            <div className="flex justify-between gap-4">
              ...
              <dd className="font-semibold">{packageLabel(session.user.interestedPackage)}</dd>
            </div>
            <div className="flex justify-between gap-4">
